...
- **可跳过 onboarding**（`runner_profiles.onboarding_skipped_at`）
- 比赛目标 CRUD（单活跃目标约束）
...
- **项目名**：`neondb`
- **Schema 同步方式**：生产用 `npm run db:push`（Vercel 不会自动迁移）
- **管理员账号**：`db:seed` 创建
...
| `race_goals` | 比赛目标（单活跃） |
| `plan_versions` | 计划版本（含 `label`） |
| `plan_workouts` | 计划课表 |
| `activities` | 训练记录（`workout_type` 为 text） |
| `daily_checkins` | 每日打卡 |
...
迁移文件（可能落后于 `db:push` 实际库）：
- `drizzle/0000_phase1_init.sql`
- `drizzle/0001_phase2_planning.sql`
- `drizzle/0002_phase4_strategy.sql`

> 注意：近期字段变更以 `db:push` 为准；迁移 SQL 未全量回写。
...
- [x] 移动端 PWA（基础版）
- [ ] 完整 i18n 覆盖所有页面
- [ ] 将 `db:push` 字段回写为正式 drizzle 迁移文件
...
*最后更新：2026-07-14 — Phase 1–4 + 导出/PWA/i18n/AI 配置说明；生产用 db:push*
