...
> 本文档面向后续接手开发的工程师与 AI Agent。所有信息基于 **2026-07-18** 项目状态。
>
...
- 管理员后台（用户启停、**备注/改用户名**、**永久注销**、邀请码、重置令牌、**概览 KPI / 用户摘要 / 限流解锁 / 审计 / 踢会话**）
- 响应式导航（移动端底栏 / 桌面侧栏**可收起**）
- **界面样式**：`apple`（默认）/ `classic`，localStorage `strideos_style`；「我的」可切换
- **全站公告**：管理端 CRUD，用户主站横幅（可关闭）
...
- **计划提示文案**：入库与展示只保留中文 `message`（`displayPlanWarning` 剥旧 `CODE: ` 前缀），不拼接英文 code
...
- **训练配速表** `/tools/paces`：近期成绩 → VDOT → E/M/T/I/R；`src/lib/tools/paces.ts`
- **心率区间** `/tools/heart-rate`：年龄/HRmax + 可选静息 → Z1–Z5；`heart-rate.ts`
- **分段配速表** `/tools/splits`：目标成绩匀速/小负分割；`splits.ts`
- **间歇课设计** `/tools/intervals`：组数×距离×配速 → 总时间/里程；`intervals.ts`
- **长跑补给** `/tools/fueling`：水/碳水/胶粗估（非医疗建议）；`fueling.ts`
- **热身放松拉伸** `/tools/recovery`：静态可勾选清单；`recovery.ts`
- **周负荷一览** `/tools/load`：读 `/plans/current` + `/activities` 聚合近 4 周，**不写库**；`load.ts`
- **比赛倒计时清单** `/tools/race-day`：勾选存 localStorage；`race-checklist.ts`
- **训练备注模板** `/tools/notes`：复制到活动备注；`notes.ts`
- **单位换算** `/tools/units`：km↔mi、配速、温度；`units.ts`
- **坡度配速修正** `/tools/grade`：经验秒/km；`grade.ts`
- **配速↔功率** `/tools/power`：平路粗模型；`power.ts`
- **VDOT 说明** `/tools/vdot`：静态文案；`vdot-guide.ts`
- **跑鞋寿命进度**：默认 700 km 阈值 + 进度条；`shoe-life.ts`（无新表）
- **存储策略**：工具扩展以 Git 内静态/纯前端为主，避免挤占 Neon 0.5G；勾选仅 localStorage
...
### UX / 管理（2026-07-15）
- **外观主题**：跟随系统 / 浅色 / 深色（`localStorage` key `strideos_theme`）；**主题色** accent（`strideos_accent` → `html[data-accent]`，默认 emerald；「我的」色板切换；覆盖 primary/ring）
...
- **用户面文案**（2026-07-16）：登录/服务错误不暴露 migrate/冷启动/DB；工具描述用产品语言；APK 对用户称「安装包」（实现细节见本文 Android 节）
...
### 管理后台运维（2026-07-16）
- 概览 KPI、用户搜索筛选与只读摘要、踢下线、限流解锁、操作审计
- 重置令牌返回可复制链接；生成时作废旧未用令牌；`/reset-password?token=` 预填
- 邀请码：过期预设、状态筛选、批量复制可用码
- 迁移 `0006_admin_ops`：`users.last_login_at` + `admin_audit_logs`
...
| 表 | 说明 |
|----|------|
| `users` | 用户（含 `admin_note`、`last_login_at`、`role`、`is_active`） |
| `sessions` | 登录会话 |
| `invite_codes` | 邀请码；**单次使用永久有效消耗**：`usedAt` 为已用标记（用户注销后 `usedByUserId` 置空但 `usedAt` 保留，不可复用） |
| `password_reset_tokens` | 密码重置令牌 |
| `auth_rate_limits` | 登录/注册/重置限流与锁定（迁移 `0005`） |
| `admin_audit_logs` | 管理端写操作审计（迁移 `0006`） |
...
- `drizzle/0006_admin_ops.sql` --- users.last_login_at + admin_audit_logs
...
- 用户暂无自助改密（文案引导联系管理员）
- 用户可见文案不暴露运维/实现细节（migrate、冷启动、WebView、占库等）；见 AGENTS §4.23
...
src/
  ...
  app/(app)/tools/
    ...
    paces/page.tsx              # 训练配速表
    pace/page.tsx               # 配速计算器
    splits/page.tsx             # 分段配速表
    intervals/page.tsx          # 间歇设计
    heart-rate/page.tsx         # 心率区间
    recovery/page.tsx           # 热身放松拉伸
    fueling/page.tsx            # 长跑补给
    load/page.tsx               # 周负荷一览
    race-day/page.tsx           # 比赛清单
    notes/page.tsx              # 备注模板
    units/page.tsx              # 单位换算
    grade/page.tsx              # 坡度修正
    power/page.tsx              # 配速功率
    vdot/page.tsx               # VDOT 说明
    bmi/page.tsx                # BMI
    race/ shoes/ strength/
  lib/
    ...
    tools/paces.ts              # 训练配速表
    tools/pace.ts               # 配速互算
    tools/splits.ts / intervals.ts / fueling.ts / heart-rate.ts
    tools/recovery.ts / race-checklist.ts / notes.ts / load.ts
    tools/units.ts / grade.ts / power.ts / shoe-life.ts / vdot-guide.ts
    tools/bmi.ts                # BMI
    tools/tools.test.ts
    tools/tools-extra.test.ts
    auth/delete-account.ts / invite-status helpers + tests
...
| GET  | `/api/v1/admin/users` | 用户列表；query `q` / `status=all|active|disabled|admin` |
| GET  | `/api/v1/admin/users/:id` | 只读支持摘要 |
| PUT  | `/api/v1/admin/users/:id` | `isActive` / `username` / `adminNote` |
| DELETE | `/api/v1/admin/users/:id` | 永久注销；确认文案；不可注销自己/唯一管理员 |
| DELETE | `/api/v1/admin/users/:id/sessions` | 踢下线 |
| GET/POST | `/api/v1/admin/invite-codes` | POST: `count` / `expiresInDays`（null=不过期） |
| DELETE | `/api/v1/admin/invite-codes/:id` | 硬删除任意码（删行） |
| DELETE | `/api/v1/admin/invite-codes` | 一键清空全部邀请码 |
| POST | `/api/v1/admin/reset-token` | 生成重置令牌；返回 resetPath/resetUrl；作废旧未用令牌 |
| DELETE | `/api/v1/admin/reset-token` | body `{ userId }` 作废未用令牌 |
| GET/DELETE | `/api/v1/admin/rate-limits` | 限流列表；DELETE body `{ id|bucket|username }` |
| DELETE | `/api/v1/admin/rate-limits/:id` | 清除单条限流 |
| GET  | `/api/v1/admin/audit-logs` | 管理员操作审计（只读） |
...
- `src/lib/auth/invite-status.test.ts`
...
- `src/lib/tools/tools-extra.test.ts`（配速表 / 心率 / 分段 / 间歇 / 补给 / 负荷 / 换算等）
...
- [x] 外观主题 system/light/dark + 主题色 accent（7 色）
- [x] 管理员用户备注与改名（`0004_user_admin_note`）
- [x] 成绩预测 / 配速计算器 / BMI（`/tools/predict|pace|bmi`，纯前端）
- [x] 工具箱 P0–P2 扩展（配速表/心率/分段/间歇/补给/恢复/负荷/清单/换算/坡度/功率/VDOT 说明；无新 DB 表）
- [x] Security: login lockout/rate-limit, headers, origin checks (0005_auth_rate_limits)
- [x] 管理后台 P0/P1：概览/用户摘要/限流/审计/重置链接（0006_admin_ops）
- [x] 用户面去开发者/运维话术（登录错误、工具描述、APK 文案）
- [x] 计划 warnings 仅中文 message
- [ ] 跑步路线记录集成（GPX / 第三方）
- [ ] 完整 i18n 覆盖所有页面
- [ ] 用户自助修改密码
